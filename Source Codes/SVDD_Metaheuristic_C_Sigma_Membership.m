...
function Inaccuracy=SVDD_Metaheuristic_C_Sigma_Membership(Input)
Inaccuracy=1;
%% Gloabl Variables
global TargetClasse Train
...
%         r = sqrt(abs(RR)); 
        ...
        %% Check Resoults for Train data
            TTarget=Train.Data(TS,:)';
            nnn=size(TTarget,2);
        ...
        Inaccuracy=1-(TP+TN)/((TP+FN)+(FP+TN));
        end
        end
        end
end
